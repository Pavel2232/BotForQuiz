...
import textwrap
from config.db import QUIZ_DICT
from keyboards.reply import get_quiz_keyboard

from config.db import redis
from utils.work_wtih_questions import get_questions, get_random_question
...
async def check_answer_handler(message: Message) -> None:
    answer = get_questions(QUIZ_DICT).get(redis.get(message.from_user.id))
...
async def conversation_handler(message: Message) -> None:
    full_answer = get_questions(QUIZ_DICT).get(redis.get(message.from_user.id))
